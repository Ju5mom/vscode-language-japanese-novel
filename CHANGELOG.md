...
- 進捗表示を原稿用紙換算（枚・行）でも扱えるように改善しました
- 原稿用紙の枚・行表示の基準を0枚開始に変更しました
    - 旧表記: 1枚19行 → 新表記：0枚19行
    - 旧表記: 1枚20行 → 新表記：1枚0行

# 3.4.15

- 原稿ビューで、閉じているフォルダーに原稿やフォルダーをドロップできるようになりました
- 原稿用紙プレビューのHTMLコメントの位置を調整（画面下に潜ってしまうのを防ぎます）
- 年や月を跨いだときに進捗が正しく動作しない問題を修正しました
